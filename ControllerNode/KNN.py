...
moveimg = 'C:\\s3\\Detected_images\\'
count = 0
ALLOWED_EXTENSIONS = {'png', 'jpg', 'jpeg'}
camera_location = 'BMSCE'
default_image_unknown="https://drive.google.com/file/d/1JsZ7WJkt7uOz2LzxF8HZxL1NPVTCIjIf/view?usp=sharing"

#Environment Variables(to be changed accordingly)

trained_model = "C:\\s3\\finalRun\\trained_model.clf"

#Django server
ngrok_endpoint = "http://14d80861251a.ngrok.io"

#Database links
ngrok_host = "6.tcp.ngrok.io" 
ngrok_port = "13723"

#Google Drive access config
access_token = "ya29.a0AfH6SMBZO8IK8rI5bVhOdDEi6B7Wu-QEVM-azivm3ytrQN075kxj_Vlt_XeGBrmrHhaAYdtz6J4wA_t8F4kMnLzgCKHDqhKrN5CqpGiL65TTKuoux3TfEMF9afk-yfQUF9v8_RBUcFGeRHgOMrdt-SYq-1Z6"
parent_folder = "1jiaXWL9EnlgOfkDj0F9TKK7Owq3RcMbE"
...
if __name__ == "__main__":
    ...
    #cam = cv2.VideoCapture("http://311394a7d4bf.ngrok.io/video_feed")  # thread1 start
    flag = True
    ...
    while True:
        ...
        if len(face_bounding_boxes) != 0:  # thread1 end
            ...
            predictions = predict(X_img, model_path=trained_model)
            if len(predictions) != 0:
                show_prediction_labels_on_image(frame, predictions)
                image_path = str(moveimg)+str(image_name)
                conn = psycopg2.connect(
                    database="criminal_records", user="postgres", password="password",host = ngrok_host,port = ngrok_port)
                cur = conn.cursor()
                ...
                for suspect in predictions:
                    if suspect[0] != "unknown":
                        ...
                        # conn.commit()
                        cur.execute("UPDATE secapp_suspect_profiles set location=%s,last_found=%s,last_pic=%s where sid=%s returning id;", (
                        ...
                        #url = urllib.request.urlopen(
                          #  ngrok_endpoint+"/getSuspectinfo/?sid="+str(suspect_id))
                        #data = json.load(url)
                        #print(data)
                        if(all_suspects != ''):
                            cur.execute("INSERT INTO secapp_suspect_recognised(sid,detect_time,detect_location,detect_pic) VALUES (%s, %s, %s, %s) returning id;", (
                                all_suspects, current_time, camera_location, imglink))
                            conn.commit()
                            id_of_new_row = cur.fetchone()[0]
                            print(id_of_new_row)
                            url = urllib.request.urlopen(
                            ngrok_endpoint+"/showRecentRecognition/?rownumber="+str(id_of_new_row))
                            data = json.load(url)
                            print(data)
